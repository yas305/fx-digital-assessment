"""
Turning an analysis into the JSON the frontend expects.

Everything here is presentation. The algorithm lives in ``dominant.py`` and
knows nothing about JSON; the HTTP layer lives in ``api.py`` and knows nothing
about how a response is built.

This module deliberately imports no web framework, so the command-line tool can
use it with nothing installed but Pillow.

There are no classes and no schema objects. Each response is a plain dictionary
built by a named function, with the JSON keys written out literally -- so to
know what the frontend receives, read the dictionary. Keys are camelCase because
that is what reads naturally in TypeScript.
"""
...
# The two counting strategies. A plain string rather than an enum class: the
# HTTP layer declares it as a Literal, which is enough for anything else to be
...
# The fewest boxes median cut will divide an image into, however few colours
# the caller asked to see. One box is never cut at all, and two rarely says
# anything useful about a photograph.
MINIMUM_BOXES = 5
...
def analyse(
    ...
    iterations = None
    if method == "mediancut":
        # At least a handful of boxes, however few colours were asked for.
        #
        # For the histogram, top_n only decides how many results to *show* --
        # the counting is the same either way. For median cut it decides how
        # many times to *cut*, which changes the answer: ask for one box and
        # nothing is ever cut, so the "dominant colour" is the average of the
        # entire image. Cutting at least a few times is what makes the answer
        # mean anything.
        run = median_cut(counted, box_count=max(top_n, MINIMUM_BOXES))
...
def explain(
    ...
    if method == "mediancut":
        run = median_cut(
            counted, box_count=max(top_n, MINIMUM_BOXES), record_steps=True
        )
